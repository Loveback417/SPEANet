...
custom_imports = dict(
    imports=['projects.SPEANet.speanet'], allow_failed_imports=False)
...
train_pipeline = [
    ...
    dict(type='mmdet.FilterAnnotations', min_gt_bbox_wh=(1e-2, 1e-2)),  # add this
    dict(
...
#Concatenate the train set and the val set.
if 'datasets' in _base_.train_dataloader['dataset']:
    for i in range(len(_base_.train_dataloader['dataset']['datasets'])):
        _base_.train_dataloader['dataset']['datasets'][i]['pipeline'] = train_pipeline
else:
