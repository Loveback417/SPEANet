...
class Hbox2HboxLayerCriterion(nn.Module):
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        if self.deep_supervision:
            ...
            deep_supervision_l = len(outputs['aux_outputs'])
        ...
        losses[f's.{deep_supervision_l}.loss_cls'] = loss_cls
        losses[f's.{deep_supervision_l}.loss_bbox'] = loss_bbox
        losses[f's.{deep_supervision_l}.loss_giou'] = loss_giou
        return losses
...
@TASK_UTILS.register_module()
class Hbox2HboxLayerMatcher(nn.Module):
