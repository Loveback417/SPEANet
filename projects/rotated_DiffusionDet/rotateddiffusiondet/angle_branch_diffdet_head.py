...
class AngleBranchDyDiffDetHead(nn.Module):
    ...
    def __init__(self,
        ...
        single_head_.update(angle_coder=angle_coder)
    ...
    def _bbox_forward(self, features, init_bboxes, init_t,
                init_features=None):
        ...
        for head_idx, single_head in enumerate(self.head_series):
            class_logits, pred_bboxes, angle_clses, proposal_features = \
                single_head(features, bboxes, proposal_features,
                            self.roi_extractor, time)
            if self.deep_supervision:
                inter_class_logits.append(class_logits)
                inter_pred_bboxes.append(pred_bboxes)
                inter_angle_cls.append(angle_clses)
            bboxes = pred_bboxes.detach()

        if self.deep_supervision:
            return torch.stack(inter_class_logits), torch.stack(
                inter_pred_bboxes), torch.stack(inter_angle_cls)
        else:
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        ...
        pred_logits, pred_bboxes, angle_cls = self._bbox_forward(x, batch_diff_bboxes, batch_time)

        output = {
            'pred_logits': pred_logits[-1],
            'pred_boxes': pred_bboxes[-1],
            'angle_cls': angle_cls[-1]
        }
        if self.deep_supervision:
            output['aux_outputs'] = [{
                'pred_logits': a,
                'pred_boxes': b,
                'angle_cls': c
            } for a, b, c in zip(pred_logits[:-1], pred_bboxes[:-1], angle_cls[:-1])]
    ...
    def prepare_training_targets(self, batch_data_samples):
        ...
        for data_sample in batch_data_samples:
            ...
            gt_bboxes = gt_instances.bboxes[..., :-1]  # N, 4, where 4 is (x,y,w,h)
            gt_angles = gt_instances.bboxes[..., -1:]  # N, 1
            h, w = img_meta['img_shape']
            image_size = gt_bboxes.new_tensor([w, h, w, h])
            norm_gt_bboxes_cxcywh = gt_bboxes / image_size  # N, 4, where 4 is (x,y,w,h)
            ...
            gt_instances.gt_angles = gt_angles
            gt_instances.masks = self.angle_coder.encode(gt_angles)  # for CrossEntropyLossCost
            gt_instances.bboxes = bbox_cxcywh_to_xyxy(
                gt_instances.bboxes[..., :-1])  # N, 4, where 4 is (x1,y1,x2,y2)
    ...
    def prepare_diffusion(self, gt_boxes, image_size):
        ...
        diff_bboxes = bbox_cxcywh_to_xyxy(x)  # num_query, 4, where 4 is (x1,y1,x2,y2)
        # convert to abs bboxes
        diff_bboxes_abs = diff_bboxes * image_size  # num_query, 4, where 4 is (x1,y1,x2,y2)

        metainfo = dict(time=time.squeeze(-1))
        pred_instances = InstanceData(metainfo=metainfo)
        pred_instances.diff_bboxes = diff_bboxes  # num_query, 4, where 4 is (x1,y1,x2,y2)
        pred_instances.diff_bboxes_abs = diff_bboxes_abs  # num_query, 4, where 4 is (x1,y1,x2,y2)
        pred_instances.noise = noise  # num_query, 4
    ...
    def predict(self,
        ...
        (time_pairs, batch_noise_bboxes, batch_noise_bboxes_raw,
         batch_image_size) = self.prepare_testing_targets(
            batch_img_metas, device)  # batch_noise_bboxes[bs,num_query,4],where 4 is (x1,y1,x2,y2)
    ...
    def predict_by_feat(self,
        ...
        ensemble_pred_logits, ensemble_pred_bboxes, ensemble_angle_cls = [], [], []
        for time, time_next in time_pairs:
            batch_time = torch.full((batch_size,),
                                    time,
                                    device=device,
                                    dtype=torch.long)
            pred_logits, pred_bboxes, angle_cls = self._bbox_forward(x, batch_noise_bboxes, batch_time)

            x_start = pred_bboxes[-1]  # bs, num_query, 4, where 4 is x1, y1, x2, y2

            x_start = x_start / batch_image_size[:, None, :]
            x_start = bbox_xyxy_to_cxcywh(x_start)  # bs, num_query, 4, where 4 is cx,cy,w,h
            x_start = (x_start * 2 - 1.) * self.snr_scale
            x_start = torch.clamp(
            ...
            if time_next < 0:
                ...
                if self.use_ensemble and self.sampling_timesteps > 1:
                    ensemble_pred_logits.append(pred_logits)
                    ensemble_pred_bboxes.append(pred_bboxes)
                    ensemble_angle_cls.append(angle_cls)
                continue
        ...
        if self.use_ensemble and self.sampling_timesteps > 1:
            # Not same as original DiffusionDet
            box_cls = torch.cat(ensemble_pred_logits, dim=2)[-1]
            box_pred = torch.cat(ensemble_pred_bboxes, dim=2)[-1] # bs, num_query, 4, where 4 is x1, y1, x2, y2
            box_angle = self.angle_coder.decode(torch.cat(ensemble_angle_cls,
                                                          dim=2)[-1]).unsqueeze(-1) # bs, num_query, 1
            box_pred = torch.cat((bbox_xyxy_to_cxcywh(box_pred),
                                  box_angle), dim=-1)  # bs, num_query, 5, where 5 is cx,cy,w,h,theta
            results_list = self.inference(box_cls, box_pred, cfg, device)

        else:
            box_cls = pred_logits[-1]
            box_pred = pred_bboxes[-1]  # bs, num_query, 4, where 4 is x1, y1, x2, y2
            box_angle = self.angle_coder.decode(angle_cls[-1]).unsqueeze(-1)  # bs, num_query, 1
            box_pred = torch.cat((bbox_xyxy_to_cxcywh(box_pred),
                                  box_angle), dim=-1)  # bs, num_query, 5, where 5 is cx,cy,w,h,theta
            results_list = self.inference(box_cls, box_pred, cfg, device)
        if rescale:
    ...
    def do_results_post_process(results_list, cfg, batch_img_metas=None):
        ...
        for results, img_meta in zip(results_list, batch_img_metas):
            ...
            results.bboxes = scale_rboxes(results.bboxes, scale_factor)
            # clip w, h
            h, w = img_meta['ori_shape']
            results.bboxes[:, 0] = results.bboxes[:, 0].clamp(min=0, max=w)
            results.bboxes[:, 1] = results.bboxes[:, 1].clamp(min=0, max=h)
            results.bboxes[:, 2] = results.bboxes[:, 2].clamp(min=0, max=w)
            results.bboxes[:, 3] = results.bboxes[:, 3].clamp(min=0, max=h)

            # filter small size bboxes
            if cfg.get('min_bbox_size', 0) >= 0:
                w, h = get_rbox_wh(results.bboxes)
    ...
    def inference(self, box_cls, box_pred, cfg, device):
        ...
        Args:
            ...
            box_pred (Tensor): tensors of shape (batch_size, num_proposals, 5).
                The tensor predicts 5-vector (x,y,w,h,theta) box
        ...
        if self.use_focal_loss or self.use_fed_loss:
            ...
            labels = torch.arange(
                self.num_classes,
                device=device).unsqueeze(0).repeat(self.num_proposals*self.sampling_timesteps,
                                                   1).flatten(0, 1)
            ...
            for i, (scores_per_image,
                ...
                scores_per_image, topk_indices = scores_per_image.flatten(
                    0, 1).topk(
                    self.num_proposals*self.sampling_timesteps, sorted=False)
                labels_per_image = labels[topk_indices]  # [num_proposal,]
                box_pred_per_image = box_pred_per_image.view(-1, 1, 5).repeat(
                    1, self.num_classes, 1).view(-1, 5)
            ...
            raise NotImplementedError('use_focal_loss must be True')

        return results

@MODELS.register_module()
class AngleBranchSingleDiffDetHead(nn.Module):
    ...
    def __init__(
        ...
        # angle.
        angle_module = list()
        for _ in range(num_angle_convs):
            angle_module.append(nn.Linear(feat_channels, feat_channels, False))
            angle_module.append(nn.LayerNorm(feat_channels))
            angle_module.append(nn.ReLU(inplace=True))
        self.angle_module = nn.ModuleList(angle_module)
        ...
        self.angle_cls = nn.Linear(feat_channels, self.encode_size)

    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        """
        :param bboxes: (N, num_boxes, 4), where 4 is (x1,y1,x2,y2)
        :param pro_features: (N, num_boxes, feat_channels)
        :param angles: (N, num_boxes, 1)
        """
        ...
        scale_shift = torch.repeat_interleave(
            scale_shift, torch.tensor(num_boxes,
                                      device=obj_features.device), dim=0)
        scale, shift = scale_shift.chunk(2, dim=1)
        fc_feature = fc_feature * (scale + 1) + shift

        cls_feature = fc_feature.clone()
        reg_feature = fc_feature.clone()
        angle_feature = fc_feature.clone()
        for cls_layer in self.cls_module:
            cls_feature = cls_layer(cls_feature)
        for reg_layer in self.reg_module:
            reg_feature = reg_layer(reg_feature)
        for angle_layer in self.angle_module:
            angle_feature = angle_layer(angle_feature)

        class_logits = self.class_logits(cls_feature)
        bboxes_deltas = self.bboxes_delta(reg_feature)
        pred_bboxes = self.apply_deltas(bboxes_deltas, bboxes.view(-1, 4))
        angle_cls = self.angle_cls(angle_feature)

        return (class_logits.view(N, num_boxes, -1),
                pred_bboxes.view(N, num_boxes, -1),
                angle_cls.view(N, num_boxes, -1),
                obj_features)
    ...
    def apply_deltas(self, deltas, boxes):
        ...
        Args:
            ...
            boxes (Tensor): boxes to transform, of shape (N, 4), where 4 is (x1,y1,x2,y2)
        Returns:
            pred_boxes (Tensor): (N,4), where 4 is (x1,y1,x2,y2)
        """
