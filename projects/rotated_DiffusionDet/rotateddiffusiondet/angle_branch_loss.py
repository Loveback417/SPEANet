...
class AngleBranchDiffusionDetCriterion(nn.Module):
    ...
    def __init__(
            ...
            loss_angle=dict(
                type='CrossEntropyLoss', use_sigmoid=True, reduction='sum',
                loss_weight=2.0)
    ):
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        loss_bbox, loss_giou = self.loss_boxes(outputs, batch_gt_instances,
                                               batch_indices)
        loss_angle = self.loss_angle_cls(outputs, batch_gt_instances,
                                         batch_indices)

        losses = dict(
            loss_cls=loss_cls, loss_bbox=loss_bbox,
            loss_giou=loss_giou, loss_angle=loss_angle)
    ...
    def loss_classification(self, outputs, batch_gt_instances, indices):
        ...
        src_logits = outputs['pred_logits']  # bs, num_query, class
        target_classes_list = [
        ...
        target_classes = torch.full(
            ...
            self.num_classes,
            dtype=torch.int64,
            device=src_logits.device)  # bs, num_query
        for idx in range(len(batch_gt_instances)):
            target_classes[idx, indices[idx][0]] = target_classes_list[idx]  # bs, num_query

        src_logits = src_logits.flatten(0, 1)  # bs*num_query, class
        target_classes = target_classes.flatten(0, 1)  # bs*num_query
        # comp focal loss.
    ...
    def loss_angle_cls(self, outputs, batch_gt_instances, indices):
        assert 'angle_cls' in outputs
        angle_cls = outputs['angle_cls']  # bs, num_query, encode_size
        target_angle_cls_list = [
            gt.masks[J] for gt, (_, J) in zip(batch_gt_instances, indices)
        ]  # [(n1,180), (n2,180), ...]

        pred_angle_list = []
        for idx in range(len(batch_gt_instances)):
            pred_angle_list.append(angle_cls[idx, indices[idx][0]])  # [(n1,180), (n2,180), ...]
        pred_angle_cat = torch.cat(pred_angle_list)  # n1+n2,180
        target_angle_cat = torch.cat(target_angle_cls_list)  # n1+n2,180

        # comp focal loss.
        if len(pred_angle_cat) > 0:
            num_instances = pred_angle_cat.shape[0]
            loss_angle = self.loss_angle(
                pred_angle_cat,
                target_angle_cat) / num_instances
        else:
            loss_angle = angle_cls.sum() * 0.

        return loss_angle

    def loss_boxes(self, outputs, batch_gt_instances, indices):
        assert 'pred_boxes' in outputs
        pred_boxes = outputs['pred_boxes']  # bs, num_query, 4
        ...
        for idx in range(len(batch_gt_instances)):
            pred_bboxes_list.append(pred_boxes[idx, indices[idx][0]])  # [(n1,4), (n2,4), ...]
            image_size = batch_gt_instances[idx].image_size
            pred_bboxes_norm_list.append(pred_boxes[idx, indices[idx][0]] /
                                         image_size)

        pred_boxes_cat = torch.cat(pred_bboxes_list)  # n1+n2,4
        pred_boxes_norm_cat = torch.cat(pred_bboxes_norm_list)  # n1+n2,4
        target_bboxes_cat = torch.cat(target_bboxes_list)  # n1+n2,4
        target_bboxes_norm_cat = torch.cat(target_bboxes_norm_list)  # n1+n2,4
...
@TASK_UTILS.register_module()
class AngleBranchDiffusionDetMatcher(nn.Module):
    ...
    def __init__(self,
        ...
        for _match_cost in match_costs:
            # todo: using string to judge focallosscost?
            if 'FocalLossCost' in _match_cost.get('type'):
                self.use_focal_loss = True
            if 'FedLoss' in _match_cost.get('type'):
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        angle_cls = outputs['angle_cls']
        batch_size = len(batch_gt_instances)

        assert batch_size == pred_logits.shape[0] == \
               pred_bboxes.shape[0] == angle_cls.shape[0]
        batch_indices = []
        for i in range(batch_size):
            pred_instances = InstanceData()
            pred_instances.bboxes = pred_bboxes[i, ...]
            pred_instances.scores = pred_logits[i, ...]
            pred_instances.masks = angle_cls[i, ...]
    ...
    def dynamic_k_matching(self, cost: Tensor, pairwise_ious: Tensor,
        ...
        while (matching_matrix.sum(0) == 0).any():
            ...
            num += 1
            if num > 999:
                raise RuntimeError('infinite loop in dynamic_k_matching assignment!!!!!!')
