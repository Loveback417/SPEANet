...
class MMRotate(LabelStudioMLBase):
    ...
    def __init__(self,
        ...
        self.model = init_detector(
            config_file, checkpoint_file, palette=palette, device=device)
    ...
    def predict(self, tasks, **kwargs):
        ...
        for item in model_results.pred_instances:
            print(f'item >>>>> {item}')
            ...
            corners = RotatedBoxes.rbox2corner(bboxes)

            for bbox, corner in zip(bboxes, corners):
                print('bbox', bbox)
                print('corner', corner)
                ...
                _, _, w, h, a = bbox[:5]
                left_top_x, left_top_y = corner[2]
                ...
                results.append({
                    ...
                    'value': {
                        ...
                        'x': float(left_top_x) / img_width * 100,
                        'y': float(left_top_y) / img_height * 100,
                        'width': float(w) / img_width * 100,
                        'height': float(h) / img_height * 100,
                        'rotation': float(a) / np.pi * 180
                    },
                    'score': score
                })
