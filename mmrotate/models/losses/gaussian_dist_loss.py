...
def gwd_loss(pred, target, fun='log1p', tau=1.0, alpha=1.0, normalize=True):
    ...
    Derivation and simplification:
        Given any positive-definite symmetrical 2*2 matrix Z:
            :math:`Tr(Z^{1/2}) = 位_1^{1/2} + 位_2^{1/2}`
        where :math:`位_1` and :math:`位_2` are the eigen values of Z
        Meanwhile we have:
            :math:`Tr(Z) = 位_1 + 位_2`

            :math:`det(Z) = 位_1 * 位_2`
        Combination with following formula:
            :math:`(位_1^{1/2}+位_2^{1/2})^2 = 位_1+位_2+2 *(位_1 * 位_2)^{1/2}`
        Yield:
            :math:`Tr(Z^{1/2}) = (Tr(Z) + 2 * (det(Z))^{1/2})^{1/2}`
        For gwd loss the frustrating coupling part is:
            :math:`Tr((危_p^{1/2} * 危_t * 危p^{1/2})^{1/2})`
        Assuming :math:`Z = 危_p^{1/2} * 危_t * 危_p^{1/2}` then:
            :math:`Tr(Z) = Tr(危_p^{1/2} * 危_t * 危_p^{1/2})
            = Tr(危_p^{1/2} * 危_p^{1/2} * 危_t)
            = Tr(危_p * 危_t)`
            :math:`det(Z) = det(危_p^{1/2} * 危_t * 危_p^{1/2})
            = det(危_p^{1/2}) * det(危_t) * det(危_p^{1/2})
            = det(危_p * 危_t)`
        and thus we can rewrite the coupling part as:
            :math:`Tr(Z^{1/2}) = (Tr(Z) + 2 * (det(Z))^{1/2})^{1/2}`
            :math:`Tr((危_p^{1/2} * 危_t * 危_p^{1/2})^{1/2})
            = (Tr(危_p * 危_t) + 2 * (det(危_p * 危_t))^{1/2})^{1/2}`
