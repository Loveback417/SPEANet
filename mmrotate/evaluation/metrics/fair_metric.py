...
class FAIRMetric(BaseMetric):
    ...
    def merge_results(self, results: Sequence[dict],
        ...
        for img_id, dets_per_cls in zip(id_list, dets_list):
            outdir = osp.join(outfile_prefix, img_id + '.xml')
            root = ET.Element('annotation')
            # size = ET.SubElement(root, 'size')
            # ET.SubElement(size, 'width').text = f'{patch.shape[1]}'
            # ET.SubElement(size, 'height').text = f'{patch.shape[0]}'
            ...
            for i, dets in enumerate(dets_per_cls):
                ...
                for qbox, score in zip(qboxes, scores):
                    bb = [f'{p:.2f}' for p in qbox]
                    cn = self.dataset_meta['classes'][i]
                    sc = f'{score.item():.4f}'
                    obj = ET.SubElement(objs, 'object')
                    res = ET.SubElement(obj, 'possibleresult')
                    ET.SubElement(res, 'name').text = cn
                    ET.SubElement(res, 'probability').text = sc
                    pts = ET.SubElement(obj, 'points')
                    ET.SubElement(pts, 'point').text = f'{bb[0]},{bb[1]}'
                    ET.SubElement(pts, 'point').text = f'{bb[2]},{bb[3]}'
                    ET.SubElement(pts, 'point').text = f'{bb[4]},{bb[5]}'
                    ET.SubElement(pts, 'point').text = f'{bb[6]},{bb[7]}'
                    ET.SubElement(pts, 'point').text = f'{bb[0]},{bb[1]}'
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as t:
            for img_id in id_list:
                outdir = osp.join(outfile_prefix, img_id + '.xml')
                t.write(outdir, osp.join('test', osp.split(outdir)[-1]))
